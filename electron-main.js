...
//  Config persistée
// ─────────────────────────────────────────────────
const CONFIG_FILE = path.join(app.getPath('userData'), 'freenekit_config.json');
...
if (fs.existsSync(CONFIG_FILE)) {
  try {
    ...
    console.log('[Freenekit] Config chargée — token:', !!appConfig.appToken, '| MACs connues:', appConfig.knownMacs.length);
  } catch (e) { console.warn('[Freenekit] Lecture config échouée:', e.message); }
}
...
//  Essaie plusieurs hôtes/IPs dans l'ordre
// ─────────────────────────────────────────────────
const DISCOVERY_HOSTS = [
  'mafreebox.freebox.fr',   // DNS universel Free
  '192.168.1.254',          // IP par défaut Freebox Revolution/Delta/Pop/Ultra
  '192.168.0.254',          // variante
  '192.168.1.1',            // variante rare
  'freebox.home',           // mDNS optionnel
];
...
async function discoverFreebox() {
  console.log('[Freenekit] Recherche de la Freebox sur le réseau...');
  for (const host of DISCOVERY_HOSTS) {
    ...
    if (info) {
      ...
      console.log(`[Freenekit] ✅ Freebox trouvée sur ${host} — ${appConfig.freeboxModel} | API ${appConfig.freeboxApiVer}`);
      saveConfig();
      return true;
    }
  }
  console.warn('[Freenekit] ⚠️  Aucune Freebox trouvée sur le réseau local.');
  return false;
}

// ─────────────────────────────────────────────────
//  HTTP vers la Freebox OS
// ─────────────────────────────────────────────────
    ...
    const req = http.request(reqOpts, (res) => {
      ...
      res.on('end', () => {
        try { resolve(JSON.parse(data)); }
          ...
          console.warn(`[Freenekit] JSON parse error ${fullPath}:`, data.slice(0, 120));
          resolve({ success: false, error: 'json_parse', raw: data.slice(0, 120) });
        }
      });
    });
    req.on('error',   (e) => { console.warn(`[Freenekit] HTTP error ${fullPath}:`, e.message); resolve({ success: false, error: e.message }); });
...
// ─────────────────────────────────────────────────
//  Authentification + cache session
// ─────────────────────────────────────────────────
let sessionExpiry = 0;
  ...
  authPromise = (async () => {
    try {
      ...
      if (!loginRes?.success || !loginRes?.result?.challenge) {
        console.warn('[Freenekit] /login/ échoué:', JSON.stringify(loginRes));
        return false;
      }
      ...
      if (sessionRes?.success && sessionRes?.result?.session_token) {
        ...
        console.log('[Freenekit] Session ouverte. Permissions:', JSON.stringify(sessionPermissions));
        return true;
      }

      console.warn('[Freenekit] Session échouée:', JSON.stringify(sessionRes));
      return false;
    } catch (e) {
      console.warn('[Freenekit] Error in authenticateSession:', e.message);
      return false;
    } finally {
      authPromise = null;
    }
...
// ─────────────────────────────────────────────────
//  Applications réseau Windows (groupées par nom)
// ─────────────────────────────────────────────────
async function getRealNetworkApps() {
  return new Promise((resolve) => {
    ...
    execPromise(`powershell -NoProfile -ExecutionPolicy Bypass -EncodedCommand ${b64}`, { timeout: 8000 })
      ...
      .catch((err) => {
        console.warn('[Freenekit] getRealNetworkApps:', err.message);
        resolve([]);
      });
  });
}
...
// ─────────────────────────────────────────────────
//  Self-test complet de tous les endpoints
// ─────────────────────────────────────────────────
async function runSelfTest() {
  const results = {};

  // 1. Découverte
  results.discovery = { ok: false };
  const info = await probeHost(appConfig.freeboxHost);
  if (info) {
    results.discovery = { ok: true, model: info.box_model_name, apiVer: info.api_version, host: appConfig.freeboxHost };
  }

  // 2. Auth
  results.auth = { ok: false };
  if (appConfig.appToken) {
    const ok = await authenticateSession(true); // force refresh
    results.auth = { ok, permissions: sessionPermissions };
  } else {
    results.auth.reason = 'no_token';
  }
  ...
  if (results.auth.ok) {
    ...
    if (r?.success && r?.result) {
      ...
      results.connection = {
        ...
        state: res.state,
        media: res.media,
        rateDown:  Math.round((res.rate_down  || 0) / 125000 * 100) / 100,
        rateUp:    Math.round((res.rate_up    || 0) / 125000 * 100) / 100,
        bandDown:  Math.round((res.bandwidth_down || 0) / 1000000),
        bandUp:    Math.round((res.bandwidth_up   || 0) / 1000000),
        ipv4:      res.ipv4,
      };
    } else {
      results.connection.error = r?.error_code || r?.error || 'unknown';
    }
  }
  ...
  if (results.auth.ok) {
    ...
    if (r?.success && Array.isArray(r.result)) {
      const connected = r.result.filter(d => d.reachable || d.active).length;
      results.lan = { ok: true, total: r.result.length, connected };
    } else {
      results.lan.error = r?.error_code || r?.error || 'unknown';
    }
  }
  ...
  if (results.auth.ok) {
    ...
    if (r?.success && r?.result) {
      ...
      results.system.error = r?.error_code || r?.error;
    }
  }

  console.log('[Freenekit] Self-test:', JSON.stringify(results, null, 2));
  return results;
}

// ─────────────────────────────────────────────────
//  Serveur Express embarqué
// ─────────────────────────────────────────────────
function startEmbeddedServer() {
  ...
  // ── Paired? ──
  server.get('/api/freebox/paired', (req, res) => {
    res.json({ isPaired: !!appConfig.appToken, model: appConfig.freeboxModel, apiVer: appConfig.freeboxApiVer });
  });

  // ── Save token (après confirmation manuelle) ──
  server.post('/api/freebox/pair/save', async (req, res) => {
    ...
    console.log('[Freenekit] Token sauvegardé. Session:', ok ? 'OK' : 'ÉCHEC');
    res.json({ success: true, sessionActive: ok });
  });

  // ── Demande d'association ──
  server.post('/api/freebox/pair/request', async (req, res) => {
  ...
  // ── Settings ──
  server.get('/api/freebox/settings', (req, res) => {
    res.json(appConfig.userSettings || {});
  });
  server.post('/api/freebox/settings', (req, res) => {
    appConfig.userSettings = { ...(appConfig.userSettings || {}), ...(req.body || {}) };
    saveConfig();
    res.json({ success: true, settings: appConfig.userSettings });
  });

  // ── Redémarrage Freebox 1-Click ──
  server.post('/api/freebox/reboot', async (req, res) => {
    ...
    if (r?.success) {
      console.log('[Freenekit] Ordre de redémarrage envoyé à la Freebox !');
      sendWindowsNotification('Redémarrage Freebox', 'Votre Freebox Server est en cours de redémarrage.');
      res.json({ success: true });
    } else {
      res.json({ success: false, error: r?.error_code || 'Impossible de redémarrer' });
    }
  });

  // ── Redirection de ports (NAT) ──
  server.get('/api/freebox/ports', async (req, res) => {
    const isAuth = await authenticateSession();
    if (!isAuth) return res.status(401).json({ success: false, error: 'Non authentifié' });
  ...
  server.post('/api/freebox/ports/add', async (req, res) => {
    const isAuth = await authenticateSession();
    if (!isAuth) return res.status(401).json({ success: false, error: 'Non authentifié' });
    const r = await freeboxFetch('/fw/redir/', { method: 'POST', body: req.body });
    if (r?.success) {
      console.log('[Freenekit] Règle NAT créée avec succès.');
  ...
  server.delete('/api/freebox/ports/:id', async (req, res) => {
    const isAuth = await authenticateSession();
    if (!isAuth) return res.status(401).json({ success: false, error: 'Non authentifié' });
    const { id } = req.params;
    const r = await freeboxFetch(`/fw/redir/${id}`, { method: 'DELETE' });
    if (r?.success) {
      console.log(`[Freenekit] Règle NAT #${id} supprimée.`);
      res.json({ success: true });
    } else {
  ...
  server.get('/api/freebox/calls', async (req, res) => {
    const isAuth = await authenticateSession();
    if (!isAuth) return res.status(401).json({ success: false, error: 'Non authentifié' });
  ...
  server.get('/api/freebox/pair/status/:trackId', async (req, res) => {
    ...
    if (r?.success && r?.result) {
      ...
      if (status === 'granted' && rawToken) {
        ...
        saveConfig();
        console.log('[Freenekit] Association accordée. Token sauvegardé.');
  ...
  // ── Self-test complet ──
  server.get('/api/freebox/selftest', async (req, res) => {
    const results = await runSelfTest();
    res.json(results);
  });

  // ── Debug brut ──
  server.get('/api/freebox/debug', async (req, res) => {
    const isAuth = await authenticateSession();
    res.json({
  ...
  // ── Données principales ──
  server.get('/api/freebox/devices', async (req, res) => {
    const isAuth      = await authenticateSession();
    ...
    if (connRes?.success && connRes?.result) {
      ...
      console.warn('[Freenekit] /connection/ échoué:', code);
    ...
    if (lanRes?.success && Array.isArray(lanRes.result)) {
      ...
      const rawDevices = all.map((d, i) => {
        ...
        if (ap && ap.tx_bytes !== undefined && ap.rx_bytes !== undefined) {
          ...
          lastDeviceStats[d.id] = {
            ...
            time:     now,
            peakDown: lastDeviceStats[d.id]?.peakDown || 0,
          };
        }
      ...
      if (newlyDiscovered.length > 0) {
        saveConfig();
        if (appConfig.userSettings?.enableNewDeviceAlert !== false) {
          const names = newlyDiscovered.map(d => d.name).join(', ');
          sendWindowsNotification('🛡️ Nouvel appareil sur votre Wi-Fi !', `Appareil connecté: ${names}`);
        }
      }
      ...
      // Alerte Surcharge
      const downPct = base.maxDownloadMbit > 0 ? (base.totalDownloadMbit / base.maxDownloadMbit) * 100 : 0;
      const threshold = appConfig.userSettings?.overloadPctThreshold || 95;
      if (downPct >= threshold && appConfig.userSettings?.enableOverloadAlert !== false) {
        sendWindowsNotification('⚠️ Surcharge Réseau', `Téléchargement à ${downPct.toFixed(0)}% de la capacité maximale !`);
      }

      console.log(`[Freenekit] LAN: ${all.length} appareils (${all.filter(h=>h.reachable||h.active).length} connectés)`);
    } else {
      const code = lanRes?.error_code || lanRes?.error || 'unknown';
      console.warn('[Freenekit] /lan/browser/pub/ échoué:', code);
    ...
    // Répartition de la bande passante PC local entre les applications
  ...
  server.listen(3001, () => console.log('[Freenekit] Serveur sur http://localhost:3001'));
}
...
app.whenReady().then(async () => {
  ...
  // 3. Fenêtre
  createWindow();
  // 4. Auth initiale si token présent
  if (appConfig.appToken) {
    const ok = await authenticateSession(true);
    console.log('[Freenekit] Auth initiale:', ok ? '✅ OK' : '❌ ÉCHEC');
  }
});
